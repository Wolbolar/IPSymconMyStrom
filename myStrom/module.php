    ...
    private function ValidateConfiguration()
    {
        ...
        if ($ipcheck == true && $mac_length == 12) {
            if ($devicetype == 0) // switch
            {
                ...
                $this->RegisterVariableFloat('power', $this->Translate('Power'), '~Watt.3680', 2);
            } elseif ($devicetype == 1) // bulb
            {
                $this->RegisterVariableBoolean('status', $this->Translate('State'), '~Switch', 1);
                $this->EnableAction('status');
                $associations = [
                    [0, $this->Translate('White'), '', 0xFFFFFF], // white
                    [1, $this->Translate('Red'), '', 0xFF0000], // red
                    [2, $this->Translate('Green'), '', 0x00FF00], // green
                    [3, $this->Translate('Blue'), '', 0x0000FF] // blue
                ];
